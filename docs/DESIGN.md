...
- **Inter, self-hosted** at `public/fonts/inter-latin.woff2` (variable, 100–900), declared in `shell.css` and in the pre-auth lock screen, and precached by the service worker so the lock screen keeps the app's own type offline. Never a font CDN: the client makes no offsite request for anything, and a webfont fetch would leak the visit to whoever served it — the same rule section 9e applies to every other third party. The licence ships with it (`licenses/Inter-OFL.txt`, SIL OFL 1.1).
...
- **The order is edited in one place, reached three ways** (`nav-edit.ts`). The phone's More tab opens the overflow sheet with the editor one row down it; the desktop rail's "Customise navigation" skips straight to the editor (a rail shows every destination, so it has no overflow list to offer); Settings' Navigation block opens the same one. **Reorder is up/down buttons, not drag** — a drag inside a scrolling sheet fights the scroll, has no keyboard equivalent and is invisible to a screen reader, and the list is short. The cut between "phone bar" and "behind More" is drawn as a labelled line rather than implied by counting rows, and both halves are named, since on a desktop the fold it describes is not visible anywhere on screen. Nothing writes until Save, so Escape discards by construction; Reset returns to registry order.
- **Settings** (`settings.ts`) — the app's own screen, pinned out of the nav order: eyebrow-headed blocks of rows for Navigation, Tiles (each with its §1b encryption tier spelled out), Alerts and Account. Every control on it does something. Signing in on a second device needs no dedicated flow at all — a synced (iCloud Keychain / Google Password Manager) or cross-device-relayed (WebAuthn's own hybrid "use a passkey on another device" flow) passkey is the same credential everywhere, so ordinary login already covers it; the device list, rename, and remote sign-out are built (section 9b). What genuinely isn't built yet — registering a second, independent passkey from an already-authenticated session, and DEK rotation for a compromised device — is stated as a fact in a note rather than rendered as a dead switch, because a screen of controls that change nothing is a screen asking to be trusted about the ones that do. Preferences live in `prefs.ts` under `shell:prefs` and sync like everything else; the alert dwell is the first one.
- **One dialog every popup is built from** (`sheet.ts`'s `openSheet()`/`sheetHead()`/`sheetFoot()`) — a native `<dialog>`, bottom sheet on a phone and a centred 560px card at 900px, one live sheet at a time (opening a second closes the first), Escape/backdrop/focus-trap from the platform rather than hand-rolled. The nav's More overflow (`shell.ts`) is the first caller; any tile's own popup — a note editor, a confirm — builds on the same `.sheet`/`.sheet-head`/`.sheet-scroll`/`.sheet-foot` classes in `shell.css` rather than inventing another treatment. The scrollport lock is keyed on `html:has(dialog[open])`, not a class an opener has to remember to add and remove.
- **In-app alerts and the bell** (`alerts.ts`, `bell.ts`) — a generic notification queue any tile can raise on (`alert({source, title, detail, href, action, sticky})`), independent of Web Push (section 6): a toast for someone already looking at the app, needs no permission, one live toast per source so a busy source can't stack the screen. History (`recent()`/`unseen()`/`markAllSeen()`) persists under `shell:alerts` and feeds the bell — a button beside the Settings gear on the phone app bar (never instead of it) and alone on the desktop top bar, badged with the unread dot, opening a panel of everything not yet seen. A `sticky` alert has no dismiss; only its own action retires it.
- **Wired end-to-end.** WebAuthn registration and login both finish for real now (challenge persisted in a short-lived `HttpOnly` cookie between `/start` and `/finish`, section 2.1), and `serveGatedBundle` (section 13.2) streams the actual built shell — `npm run build:app` (`scripts/build-app.mjs`, esbuild) bundles `src/app/shell/main.ts` and `shell.css` to `dist/app/*.txt`, which `src/worker/index.ts` imports as plain text and serves once `validateSession` passes. The DEK is generated (registration) or unwrapped (login) client-side in `public/shell/auth.js` *before* the gated bundle is ever fetched — necessarily so, since the wrapped DEK has to travel in the same request that proves the passkey — and handed to `main.ts`'s `start({ dek })` directly, so the gated bundle itself never touches the master key or the wrapping step. Registration is bootstrap-only for now: the Worker accepts `/auth/register/start` only while zero accounts exist. That gate has nothing to do with adding a device, though — a synced or cross-device-relayed passkey is the same `credential_id` wherever it's presented, so ordinary login already handles a second device with no registration involved at all, and the recovery phrase (section 2.1/10.3) handles a passkey reachable from nowhere. What the gate is actually blocking is narrower: registering a genuinely distinct, independent passkey for an already-authenticated session (a hardware key, or deliberately not relying on platform sync) — not yet built, and not the same thing as "a new device."
